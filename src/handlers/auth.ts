...
const parseEventBody = (eventBody: string | undefined) => {
  if (!eventBody) {
    return {};
  }
...
export const register: APIGatewayProxyHandlerV2 = async (event) => {
  ...
  try {
    ...
    log('error', 'register_failed', { err: (err as Error).message });
...
export const login: APIGatewayProxyHandlerV2 = async (event) => {
  ...
  try {
    ...
    log('error', 'login_failed', { err: (err as Error).message });
...
export const refresh: APIGatewayProxyHandlerV2 = async (event) => {
  ...
  try {
    ...
    log('error', 'refresh_failed', { err: (err as Error).message });
    return response(401, { message: 'Invalid refresh token' });
  } finally {
