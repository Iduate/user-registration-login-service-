...
const getSecretBytes = async (): Promise<Uint8Array> => {
...
export const signRefreshToken = async (payload: { sub: string; tokenId: string }) => {
  const secret = await getSecretBytes();
  const now = Math.floor(Date.now() / 1000);
...
const verifyToken = async (token: string): Promise<BaseClaims> => {
  const secret = await getSecretBytes();
  try {
    ...
    return payload as BaseClaims;
  } catch (err) {
    log('warn', 'jwt_verify_failed', { err: (err as Error).message });
    throw err;
  }
};

type AccessClaims = BaseClaims & { token_use: 'access' };
type RefreshClaims = BaseClaims & { token_use: 'refresh'; jti: string };
